...
| 노선 | 설명 |
|---|---|
| `cel` | **기본값.** 선을 먼저 긋고 그 아래를 색면으로 채웁니다. 색이 선 밖으로 새지 않고 빈틈도 없습니다. **원화에 선이 없고 양옆 색이 뚜렷이 갈리는 경계는 선을 긋지 않고 색면끼리 맞물리게 둡니다** — 사람이 손으로 그릴 때 하는 판단입니다. 값어치를 하는 레이어만 놓기 때문에 레이어 예산을 억지로 다 쓰지는 않습니다 |
...
그리고 스폰서 문법의 로고 무리 — 리어 범퍼의 작은 **ForzaSqueegee 워터마크**
(기본 켬)와, 넣어 준 로고 이미지를 옆면 로커 위 한 줄·리어·프론트에 앉힌 것.
로고는 미러하지 않습니다(반대편은 자리만 거울). 글자는 캐릭터 이름을 넣어 준
때만 섭니다(`--text`).

**스타일 프리셋**(`--style`, 편집기 창의 드롭다운)은 사람이 만든 리버리 30벌에서
읽은 문법 다섯 — 레이싱 스폰서(띠 3~4 + 로고 줄 + 번호) · 무늬·꽃(파스텔 바탕 +
테마 산포) · 스플래시·찢김(무늬 도형 가장자리) · 미니멀(색면 하나 + 워드마크) ·
다크 그래피티(검정 바탕 + 형광 액센트 + 큰 워드마크) — 이고, 기본(자동)은 계열
후보를 다 지어 점수로 고릅니다. 프리셋은 기본값의 묶음이라 바탕 색·글자
스타일·면 배정을 직접 정하면 그것이 앞섭니다.

주요 옵션: `--style`(위 프리셋 — racing · floral · splash · minimal · dark) ·
`--text 이름`(+`--subtext` · `--text-number`) · `--no-deco`(꾸밈 빼고 도안만) ·
`--base #RRGGBB`(베이스 색 지정) ·
