...
| 0004 | 구성 계열 · 캐릭터 이름 글자 메뉴 | [Itasha] 메뉴에 둘을 더한다. ① [Composition Family] — 옆면 꾸밈의 구성 계열(자동 · minimal · graphic_bed · diagonal_flow · motorsport · splash)을 못 박는다 (`flsedit family --composition`). ② [Character Name Text...] — 이름·작품명·스타일·자리·우선순위·테두리·그림자·레이어 상한·게임 글꼴 폴백을 받는 대화상자 (`flsedit text --text …`), [Remove Text]는 `no-text`. 마지막 입력은 QSettings `itasha/text/*`에 남아 다음에 다시 뜬다. 문자열은 `QProcess` 인자 하나로 그대로 넘어가 띄어쓰기가 산다. |
