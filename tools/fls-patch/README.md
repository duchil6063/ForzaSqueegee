...
| 0011 | [Auto Decoration...] 창 **스크롤** · 빈 번호 칸은 번호 없음 | 묶음이 여섯이라 창이 화면보다 커졌다 — 본문을 QScrollArea에 넣고 [짓기]·[취소]는 아래 고정, 첫 높이 720px. 레이싱 번호 칸은 값을 **늘** 넘긴다(`--text-number ''`) — 비우면 조리법에 남아 있던 번호도 지워진다 (사용자 지시 2026-09-03). |
| 0012 | 바탕 도색 견본이 **도안에서 고른 색**을 보인다 | '도안에서' 모드에서 견본이 늘 흰색이었다 — 엔진이 고른 색은 조리법에 없고 굽기 결과에만 있었다. `flsedit state`가 `paint_auto`(엔진 `studio._auto_paint`)를 주고, 창은 그 색을 견본에 "(도안에서 고름)" 표시와 함께 보인다(직접 고르기로 바꾸면 종전대로). |
