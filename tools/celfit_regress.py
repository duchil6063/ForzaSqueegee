...
def palette_level() -> None:
    ...
    # **상한 접기** (분해 단계 — 영역 색·획 색): 상한까지만 줄고, 경계 잠금은
    # 상한이 남아도 안 넘는다. 넓이 가중이라 큰 항목이 대표를 잡는다.
    cols2 = [(40, 34, 36), (43, 37, 39), (46, 40, 42), (200, 190, 185),
             (204, 194, 189), (208, 198, 193), (120, 60, 60), (60, 120, 60)]
    w2 = [100.0, 1.0, 1.0, 100.0, 1.0, 1.0, 50.0, 50.0]
    t, st = RA.fold_palette(cols2, w2, [], k_max=4, move=12.0)
    check(st["before"] == 8 and st["after"] == 4,
          f"상한 4로 접힌다 ({st['before']}→{st['after']})")
    check(t[(43, 37, 39)] == t[(40, 34, 36)] and t[(40, 34, 36)] != t[(200, 190, 185)],
          "가까운 어두운 셋이 한 색, 밝은 무리와는 따로")
    check(sum(abs(a - b) for a, b in zip(t[(40, 34, 36)], (40, 34, 36))) <= 3,
          f"무게 100이 대표를 잡는다 {t[(40, 34, 36)]}")
    t2, st2 = RA.fold_palette(cols2, w2, [(0, 1)], k_max=2, move=12.0)
    check(t2[(40, 34, 36)] != t2[(43, 37, 39)],
          "경계 잠금 쌍은 상한이 남아도 안 묶인다")
    check(st2["after"] >= 3, f"잠금 탓에 상한 2까지 못 내려간다 ({st2['after']}색)")
    t3, _ = RA.fold_palette(cols2, w2, [(0, 1)], k_max=2, move=12.0)
    check(t2 == t3, "상한 접기 두 번이 같다")
    _t4, st4 = RA.fold_palette(cols2, w2, [], k_max=0, move=12.0)
    check(st4["after"] == 4, f"상한 0이면 반경만 접는다 ({st4['after']}색)")
