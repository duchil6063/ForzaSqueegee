...
def check() -> int:
    ...
    print(msg("있다 — {path} ({n:,}파일 · {mb:.1f}MB){warn}",
              path=DEST, n=n, mb=size / 1e6,
              warn="" if n >= FILES else
              msg("  ※ {want:,}파일이어야 한다 — 다시 받을 것", want=FILES)))
...
def verify() -> int:
    """받아 둔 것이 **고정 커밋 그대로인가**."""
    if not DEST.is_dir():
        print(msg("없다 — {path}", path=DEST))
        return 1
    n, got = aggregate(DEST)
    ok = n == FILES and got == AGG
    print(msg("{status} — {n:,}파일 · {digest}…",
              status=msg("맞다") if ok else msg("어긋난다"),
              n=n, digest=got[:16]))
    if not ok:
        print(msg("  기대: {want:,}파일 · {digest}…\n"
                  "  다시 받으려면: python tools/get_kfps.py",
                  want=FILES, digest=AGG[:16]))
...
def _fetch_git(work: Path, log) -> Path | None:
    ...
    for cmd in cmds:
        ...
        if r.returncode != 0:
            log(msg("  git 실패 ({cmd}) — {err}",
                    cmd=' '.join(cmd[1:3]), err=r.stderr.strip()[:200]))
            return None
    got = work / SUB
...
def _fetch_zip(work: Path, log) -> Path | None:
    ...
    """
    blob = work / "kfps.zip"
    log(msg("  zip을 받는 중… ({url})", url=ZIP_URL))
    ...
    with zipfile.ZipFile(blob) as z:
        ...
        if not names:
            log(msg("  zip 안에 {sub}가 없다", sub=SUB))
            return None
...
def fetch(log=print) -> int:
    ...
    size = sum(p.stat().st_size for p in DEST.rglob("*") if p.is_file())
    log(msg("받았다 — {path} ({n:,}파일 · {mb:.1f}MB)",
            path=DEST, n=FILES, mb=size / 1e6))
    return 0
...
def main() -> int:
    ...
    if a.verify:
        return verify()
    if have():
        print(msg("이미 있다 — {path}  (다시 받으려면 폴더를 지우고 실행할 것)",
                  path=DEST))
        return 0
    return fetch()
