...
# ---- 인물이 써도 되는 자리의 상한 (2026-09-03 공유 리버리 corpus 실측) ----
# 사람 판 30벌(작가 15인 · 옆면 50장)의 주역 덩어리를 그 차의 옆면 뼈대(로커·
# 벨트 무릎·휠아치)에 대고 쟀다 (`ruler._clusters` 덩어리의 p1~p99 상자, 작가
# 단위 중앙값). 주역은 **휠아치를 넘어** 앞뒤 펜더까지 눕는다 — 인물 상자 ÷
# 휠아치 사이 구간(문짝): 중앙 1.44 (p25 0.93 · p75 1.63). 다리가 아치에
# 잘리는 것은 문법이다. 실제로는 높이 예산이 먼저 물려 이 폭은 상한으로만 선다.
PERSON_DOOR_FILL = 1.44
# 휠아치를 못 읽는 면의 폴백 — 도색 상자 폭의 몫 (같은 corpus의 인물 상자 ÷
# 옆면 도색 상자 폭: 중앙 0.65, p25 0.48 · p75 0.77)
PERSON_SPAN_MAX = 0.65
# **인물 잉크 상자 ÷ 차체 밴드(로커~벨트 무릎)**. 같은 corpus에서 인물 윗변은
# 밴드 높이의 **1.29배**(p25 1.18 · p75 1.40) — 아랫변은 로커(0.01). 벨트 위로
# 나간 몫은 넓이의 11%(p25 8% · p75 15%)이고 그 71%가 유리 자리라 게임이 안
# 그린다 — 사람은 머리카락 끝·어깨·팔이 잘리는 것을 감수하고 크게 앉힌다.
# 얼굴은 다르다: 벨트 위로 올라가도 되는 것은 머리카락뿐이고 머리 상자는 벨트
# 아래에 잡는다 (`compose.place.person_scale` · `Look.head`). 넘긴 몫은 이웃
# 면으로 안 간다 (사용자 지시 2026-08-27) — 유리까지 그리려면 편집기에서 도안을
# 벨트라인으로 **가르고** 위쪽 반을 유리 면에 올린다.
# A/B (33판, `work/lab/deco` W10T→W11F): 0.96 → 1.27 + 얼굴 자에서 주역 몫
# hero_share 0.15 → 0.20(사람 0.21~0.52), 벨트 위 몫 0.5% → 10%(사람 11%), 차 H
# 0.637 → 0.695, 얼굴 자가 물린 판 18/33(2~24% 축소). 얼굴 자 없이(W11H) 0.24까지
# 가나 버스트의 눈이 벨트에 걸린다. 폭만 키운 판(W11W)은 해시가 하나도 안 갈렸다 —
# 높이가 유일한 병목이다.
PERSON_BAND_FILL = 1.27
