...
class Driver:
    ...
    def edit_menu_last_row(self, confirm: bool = False) -> None:
        """**지금 선택된 레이어 줄의 편집 메뉴에서 마지막 행을 고른다.**

        `+` 칸을 안 거친다 — 불러온 그룹은 리스트가 `1 - 3,000` 한 줄로 접혀
        있고 개별 레이어로 풀어도 `+`는 3,000줄 위라 `goto_plus`가 못 간다
        (실측: `home`은 한 번 튀고 반복이 안 되고, `up`은 한 칸씩이다). 편집
        메뉴는 **아무 줄에서나** 열리므로 그쪽이 유일하게 서는 길이다.

        마지막 행이 무엇인지는 줄의 종류가 정한다 — 접힌 그룹 줄이면
        **그룹 해제**, 보통 레이어 줄이면 **모든 레이어 삭제**다. 그래서 이
        하나로 둘 다 한다. 아래 이동은 마지막 행에서 클램프하므로 충분히
        누르기만 하면 된다 (행 수를 안 센다 — 메뉴가 바뀌어도 안 깨진다).
        """
        for _ in range(4):
            gio.press("enter")
            time.sleep(0.5)
            if self._edit_menu_open():
                break
            gio.press("esc")
            time.sleep(0.4)
        else:
            raise DriverError("레이어 편집 메뉴를 못 열었다")
        for _ in range(12):                  # 클램프까지 내린다
            gio.press("down")
            time.sleep(0.12)
        time.sleep(0.3)
        gio.press("enter")
        ...
        if confirm:
            for _ in range(8):
                if self._confirm_band():
                    gio.press("enter")
                    time.sleep(0.8)
                    return
                time.sleep(0.25)
            raise DriverError("확인창이 안 떴다")

    def delete_all_layers(self) -> None:
        """캔버스 전체 삭제 — 만들기 메뉴 마지막 행('모든 레이어 삭제') → 확인창 Enter=예.

        실측: 선택 행은 검은 배경 + lime 테두리로 렌더(흰 행 아님). 아래 이동은
        마지막 행에서 클램프. 확인창 = lime 제목 밴드(y 432~500), Enter=예/Esc=아니요.

        `+` 칸에 못 가는 판(불러온 그룹)에서는 **편집 메뉴 쪽으로 돌아간다** —
        거기 마지막 행도 '모든 레이어 삭제'다 (`edit_menu_last_row`).
        """
        try:
            self._open_create_menu()
        except DriverError:
            self.edit_menu_last_row(confirm=True)
            return
        for _ in range(10):  # 마지막 행으로: 선택 밴드가 모든 흰 밴드보다 아래
            img = self.cap()
            bands, sel = self._menu_bands(img)
            if sel is not None and bands and sel[0] > bands[-1][1] - 5:
                break
            gio.press("down")
            time.sleep(0.3)
        else:
            raise DriverError("모든 레이어 삭제 행 이동 실패")
        ...
        self._step("enter", confirm_open, "모든 레이어 삭제 확인창")

        def back_to_list() -> bool:
            img = self.cap()
            return self.list_selection(img) is not None and self.plus_selected(img) is True

        self._step("enter", back_to_list, "삭제 후 리스트 복귀")
