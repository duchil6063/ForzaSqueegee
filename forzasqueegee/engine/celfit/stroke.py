...
# 폭 바닥을 **전단 없는 후보에도** 거나 (기본 꺼짐 — 아래 실측으로 기각).
#
# 물음은 "사람 판의 초승달(`A_36` 20.4%)이 우리에겐 0.3%인 까닭이 획이 너무
# 가늘어서인가"였다. 켜면 원화 띠보다 이만큼 넘게 가는 도형이 후보에서 빠지므로
# 실오라기 어휘(`U_45` 최소 폭 0.17px)가 띠 굵기를 대신 쓰는 자리가 막힌다.
#
# 실측 (04판, 끔 ↔ 켬): 폭비 0.780 → **0.994** (놓인 폭이 원화 띠와 같아진다) ·
# 뾰족한 끝 0.0403 → **0.0091** (−77%) — 여기까지는 좋다. 그런데
# **U_45가 29.6% → 60.2%로 두 배가 되고 A_36은 0.59% → 0.40%로 준다.**
# 바닥은 어휘를 바꾸지 않는다 — 같은 실오라기를 제 폭까지 키워 쓸 뿐이다.
# 대가는 획 도형 +26.5% · 장수 +9.9% · 원화 RMSE +2.1%.
#
# 그러니 초승달이 안 서는 까닭은 폭이 아니다. 남은 후보는 **마디 길이**다 —
# 테이퍼는 굽은 획을 한 장으로 덮을 때 값을 하는데 우리는 획당 1.66장으로
# 잘라 쓴다 (`shapes_per_stroke`).
#
# 폭비·뾰족 끝만 따로 벌고 싶으면 이 스위치가 그 값을 낸다 (장수를 그만큼 낸다).
