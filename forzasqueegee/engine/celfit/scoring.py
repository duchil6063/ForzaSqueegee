...
class _Scorer:
    ...
    def __init__(self, cat: Catalog, upp: float, w: int, h: int,
        ...
        if seam and _SEAM_PULL > 0.0:
            ...
            #
            # §9의 당김은 **먼저 그린** 이웃 쪽으로만 걸렸다. 근거는 "나중 면이
            # 이음을 가져간다"였고 그 자체는 맞다 — 다만 그것은 이 면이 **나중**
            # 일 때의 이야기다. 이 면이 **먼저**일 때 반대쪽(나중에 그릴 이웃)
            # 위의 물림은 벌점만 면제돼 있고 이득이 0이라, 하강이 거기 갈 이유가
            # 없다. 그래서 이 면은 제 경계에서 멈추고, 나중 면도 제 경계에서
            # 멈추고, 그 사이 양자화 몫이 **빈 자리**로 남는다 (실측 S1-09:
            # 봉인 전 미커버 4,077표본 — 태반이 그 자리다).
            #
            ...
            if celaxes.on("OVERPAINT"):
                ...
                #
                # 깊이가 앞뒤로 다른 것이 이 축의 뜻이다: 먼저 그린 이웃 쪽은
