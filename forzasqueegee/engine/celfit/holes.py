...
def _growable(cat: Catalog, lay: Layer, ink_long: bool = False) -> bool:
    ...
    종전에는 `("A_01", "A_02", "A_22")` 세 종만 키웠다. 그 목록의 뜻은 "단일
    고리·불투명 도형"이었는데, 채움 어휘 76종이 **전부** 그렇다
    (`vocabulary._FILL_ALL` — 단일 루프·불투명이 그 어휘의 선발 조건이다).
    세 종만 열어 두면 방패·삼각·초승달로 그린 면은 경계 부스러기를 공짜로
    못 먹고 메움 도형을 부른다 — 실측 S0-09에서 봉인이 성장 13회에 그치고
    268장을 샀다. 판정을 목록에서 카탈로그로 옮긴다.
