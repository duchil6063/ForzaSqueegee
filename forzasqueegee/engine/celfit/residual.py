...
무조건 기각한다. 종전에는 그 패스가 **맨 마지막**에 돌았다: 구멍 메움과 잔차
수리가 먼저 도형을 사 버린 뒤였다. 여기서는 같은 기계를 **잔차 자리에 초점을
맞춰 먼저** 돌린다. 그 한 번의 순서 바꿈이 ⑥을 최후 수단으로 만든다.

이 모듈이 직접 도형을 사지는 않는다 — 진단(`analyze`)과 초점(`focus_layers`)만
낸다. 사는 일은 종전대로 `holes.fill_holes`·`repair.repair_mismatch`가 하고,
이 단을 지난 뒤라 살 것이 줄어 있다.
"""
