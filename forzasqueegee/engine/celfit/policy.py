...
class RoutePolicy:
    ...
    #
    # 획(선)과 면(채움)을 **반드시 따로** 켠다. 짐작이 아니라 실측이다
    # (2026-09-01 독립 A/B): 획만 켜면 총 도형 +0.4~0.8%, 면만 켜면 −1.2%로
    # 부호가 갈리고, **둘을 같이 켜면 어느 쪽보다도 나쁘다** (보이는 오차
    # +12.5% · 접합 끊김률 +29.2%, 전부 3/3 — 두 팔의 합보다 나쁘다).
    # 그래서 통합 스위치 하나의 결과로 세부 기능을 기각하면 안 된다.
    # 근거는 `affine` 문서의 팔별 표. 셋 다 기각이라 기본은 꺼짐이다.
    #
