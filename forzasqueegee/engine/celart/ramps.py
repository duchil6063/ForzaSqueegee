...
3. **예산** (§22) — 64·96·128·160 네 후보를 지어 재현 오차가 문턱 안에 드는
   가장 작은 것을 고른다. 정확한 숫자를 목표로 삼지 않는다.
...
#
# 반경의 근거는 새 상수가 아니라 **저장소가 이미 쓰는 JND 바닥**이다:
# `celart.marks._MARK_DE = 4.0`(Lab ΔE) 아래는 "안 보이는 차이"로 이미
# 취급하고 있고, 보이는 오차 자(`metrics.imp_error_seen`)도 그 아래를 0으로
# 센다. 그 바닥의 4분의 3을 반경으로 쓴다 — 옮긴 색이 그 자리에서 새로
# 문턱을 넘지 않게.
from .marks import _MARK_DE as _JND

MAX_MOVE_DE = float(os.environ.get("FS_RAMP_MOVE_DE", 0.75 * _JND))
