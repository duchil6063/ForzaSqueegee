...
# **복잡도 값 배수** — 병합에서만 λ에 곱한다 (배치·가격이 쓰는 λ는 안 건드린다).
# 분해 단계의 복잡도-충실도 파레토를 재는 손잡이다.
#
# 실측 파레토 (표준 7장 · 분해만 · `work/lab/decotab.py`, 중앙값):
#
#   배수  영역   색    JND색  rmse_src  유령경계  지운경계
#   ×1    413   408   142    17.708    0.0029    0.745
#   ×2    385   378   137    17.731    0.0029    0.747
#   ×4    344   336   132    17.745    0.0028    0.750
#   ×8    319   310   125    17.785    0.0030    0.762
#   ×16   285   278   113    17.815    0.0038    0.769
#   ×32   245   238   103    17.879    0.0036    0.772
#   ×64   205   197    80    18.039    0.0042    0.783
#
# 무릎은 ×8~×16이다 — 거기서 유령 경계가 0.0030 → 0.0038로 뛰고(+27%) 오차의
# 기울기가 선다. ×64까지 가도 원화 RMSE는 +1.9%뿐이고 영역·색이 반으로 준다.
#
# 다 구운 판에서도 같다 (표준 3장 · `work/lab/lamtab.py`): ×8에서 영역 −24.5% ·
# 색 −5.9% · 장수 −0.9%인데 **원화 RMSE·커버리지가 소수점 둘째 자리까지 그대로**다.
# 즉 지금 λ는 **덜 합친다** — 저 영역들은 애초에 JND 안이라 지워도 화면이 안 바뀐다.
#
# 그런데도 기본을 1.0으로 둔다: 벌 수 있는 것이 장수 −0.9%뿐이라(팔레트는 획 색이
# 정한다 — `ink-has-no-label-privilege`) 병합 λ와 배치 λ가 같은 자라는 규약을
# 맨 배수로 깨뜨릴 값을 못 한다. 그 규약을 고치려면 병합의 **도형 수 추정**을
# 고쳐야 한다 (영역 하나가 줄어도 도형이 그만큼 안 준다는 것이 위 실측이다).
