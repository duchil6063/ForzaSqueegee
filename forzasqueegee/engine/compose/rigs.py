...
def _place_for(name: str, lk: Look, maps: dict[str, gsurf.SurfaceMap],
    ...
    if smap is not None and not smap.uncertain:
        ...
        if got is not None:
            got.surface = name
            return got
        notes.append(f"{name}: 도색 마스크에 도안 비율({lk.aspect:.2f})이 안 들어갔다 "
                     f"— 프리셋으로 앉힌다")
    elif smap is None:
        notes.append(f"{name}: 실측 지도가 없다 — 프리셋")
    else:
        notes.append(f"{name}: 실측이 의심스럽다({smap.note}) — 프리셋")
    p = dict(preset.get(name, {"x": 0.0, "y": 0.0, "scale": 0.25, "rot": 0.0}))
    return Place(surface=name, plan=Path(), x=p["x"], y=p["y"],
                 scale=p["scale"], rot=p.get("rot", 0.0), why="프리셋")
...
def surfaces_for(car: str | None, media: str | None = None,
    ...
    **실측이 하나 더 있다**: 설치 마스크가 "칠할 수 있다"고 말하는 자리 중
    ...
    if car and media and not maps and not probe_ok(car, media):
        notes.append(f"실측 지도({car})는 못 박은 차({media})의 것이 아니다 — "
                     f"안 쓴다 (설치 지도만)")
    try:
        ...
        if media is None and car:
            ...
            if media and side_w:
                ...
                if plain and plain != media:
                    notes.append(
                        f"이름은 {plain}을(를) 골랐지만 실측 옆면 {side_w:.0f}유닛과 "
                        f"격이 다르다 ({gcars.size_text(plain) or '크기 모름'}) — "
                        f"{media}로 간다")
            if media is None and cands:
                ...
                # 902는 사람이 한눈에 가른다.
                top = ", ".join(f"{m}({s:.1f}, {gcars.size_text(m) or '크기 모름'})"
                                for s, m in cands[:3])
                notes.append(f"설치 차량 매칭이 애매하다 (후보 {top}) — 실측 "
                             f"지도만 쓴다. 못 박으려면 `--media <이름>`")
        if media:
            imaps = carfiles.surface_maps(media)
            maps.update(imaps)
            notes.append(f"설치 파일 면 지도를 쓴다 ({media} — 면 {len(imaps)}개)")
            ...
            if blank:
                notes.append("·".join(blank) + " 마스크가 백지다 (실루엣 없음) — "
            ...
            notes.append("게임 설치 폴더를 못 찾았다 — 면 지도 없이 프리셋으로 "
                         "앉힌다. `python -m forzasqueegee gamedir <경로>`로 "
                         "못 박을 것")
    except Exception as e:                         # 설치본이 없어도 죽지 않는다
        notes.append(f"설치 파일 면 지도 불가 ({e}) — 실측 지도만 쓴다")
    top = maps.get("top")
    if top is not None:
        bands = gseam.top_glass(top, probe.get("top"))
        if bands:
            top.drawn = gseam.top_body(top, bands)
            notes.append("윗면 유리를 실측으로 뺀다 (프로브): "
                         + " · ".join(f"u {a:.0f}~{b:.0f}" for a, b in bands))
        elif probe.get("top") is not None:
            notes.append("윗면 프로브가 있지만 유리를 못 갈랐다 — 윗면을 통째로 쓴다")
    return maps
    ...
    **여기가 '유리에 안 그려진다'를 처음으로 아는 자리다.** 설치 마스크의 옆면은
    ...
    for name in ROLE_MAIN:
        ...
        try:
            ...
            notes.append(f"{name}: 옆면 뼈대를 못 읽었다 ({e}) — 설치 마스크 그대로")
            continue
        if geom.body_height <= 8.0:
            notes.append(f"{name}: 벨트라인이 이상하다 (차체 높이 "
                         f"{geom.body_height:.0f}유닛) — 설치 마스크 그대로")
            continue
        ...
        if win is not None and not win.uncertain and win.mask.size > 1:
            try:
                ...
                notes.append(f"{name}: 유리 이음새 계산 실패 ({e})")
    ...
    if est_used and est_arch:
        ...
        notes.append("휠아치가 마스크에 없다 — 로케이터 어림으로 자리를 뚫고 피한다 "
                     f"(범퍼 기반 배율 {k:g}유닛/m · 자리 오차 중앙 0.11 m)")
    if out:
        first = next((r for r in out.values() if r.parts), None)
        if first is not None:
            what = " · ".join(f"{lab}(u={u:.0f})" for u, _v, lab in first.parts)
            notes.append(f"부품 자리를 잰다 (설치 파일 로케이터): {what} — "
                         f"얼굴이 그 위에 안 앉게 민다")
        g = next(iter(out.values())).geom
        hv = g.roof - g.sill
        notes.append(
            f"옆면 뼈대: 로커 {g.sill:.0f} · 벨트라인 {g.belt:.0f} · 루프 "
            f"{g.roof:.0f} (차체는 벨트라인 아래 {g.body_height / max(1e-6, hv) * 100:.0f}%"
            + (f", 유리 이음새 겹침 {next(iter(out.values())).seam.iou:.2f}"
               if next(iter(out.values())).seam else ", 유리 면 없음") + ")"
