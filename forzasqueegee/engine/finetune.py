...
#
# **이 축은 셀 도안 크기에서 사실상 안 돈다** (2026-09-01 독립 A/B). 이 패스만
# 켠 판(`FS_SKEW=1 FS_SKEW_STROKE=0 FS_SKEW_FILL=0`, 표준 01·04·06)이 기준판과
# **모든 지표에서 같았다** — 전단 레이어 0장이다. 막는 것은 `step_visible`이고,
# 그것이 옳게 작동한 결과다: 한 스텝(0.01)이 미는 거리가 `0.01·|sy|·ext_y`인데
# 채움 레이어의 `|sy|` 중앙이 0.06~0.07이라 그 거리가 0.08유닛 — 이동
# 양자(0.5)의 6분의 1이다. 축을 밟아 볼 자격이 있는 레이어가 판당 0.5~1.0%
# (10~20장)뿐이고, 그중 어느 것도 개선을 못 냈다.
#
# 그래서 `plan.finetune.skew_layers`를 "이 패스가 만든 전단"으로 읽으면 안 된다
# — 그 수는 패스가 끝난 시점 플랜 전체의 전단 장수다 (`refine_plan` 반환값과
# 같은 자). 스테이지 귀속은 이름이 아니라 **스위치로 끄고 다시 굽는 것**만이 낸다.
