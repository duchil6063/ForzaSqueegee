...
def build_parser() -> argparse.ArgumentParser:
    ...
    # "오류: …"로 찍는다.
    p_it.add_argument("--motif", default=None, metavar="계열",
                      help="꾸밈 모티프 계열을 직접 고른다 — star · flower · "
    ...
    p_it.add_argument("--flip", action="store_true",
                      help="도안을 좌우반전한다 (= 오른쪽 면을 원본 그대로 쓴다). "
                           "인물을 눕히면 그림의 좌우축이 세로로 서므로 이 한 "
                           "비트가 인물이 어느 옆구리를 바닥에 두고 눕는지를 "
                           "가른다 — 얼굴이 땅을 보면 이걸 준다")
    p_it.add_argument("--preset-only", action="store_true",
                      help="실측 지도를 안 쓰고 프리셋 상수로만 앉힌다")
    p_it.add_argument("--make-only", action="store_true",
                      help="구성 파일만 짓고 끝낸다 (게임을 안 건드린다)")
    p_it.add_argument("--dry-run", action="store_true",
                      help="검증하고 계획만 보여 준다 (게임을 안 건드린다)")
    p_it.add_argument("--restart", action="store_true",
                      help="진행 파일을 버리고 처음부터")
    p_it.add_argument("--no-prepare", action="store_true",
                      help="그룹 준비를 건너뛴다 (이미 게임에 저장돼 있을 때)")
    p_it.add_argument("--keep-existing", action="store_true",
                      help="면에 이미 있는 레이어를 안 지운다 (기본은 비우고 "
                           "올린다 — 안 그러면 두 번째 실행이 예산 초과로 막힌다)")
    p_it.add_argument("--no-autofit", action="store_true",
                      help="올린 뒤 화면으로 재서 스스로 맞추는 단계를 끈다 "
                           "(실측 지도가 있는 면에서만 도는 단계다)")
    p_it.add_argument("--yes", action="store_true",
                      help="'현재 자동차에 적용'을 묻지 않고 진행한다 "
                           "(지금 차량의 디자인을 덮는다)")

    p_gd = sub.add_parser("gamedir",
                          help="FH6 설치 폴더를 보여 주거나 못 박아 저장한다 "
                               "(자동 탐색이 못 찾을 때)")
    p_gd.add_argument("path", nargs="?", default=None,
                      help="설치 폴더 — `media/Cars`가 있는 곳. 안 주면 지금 "
                           "쓰는 자리만 보여 준다")
    p_gd.add_argument("--clear", action="store_true",
                      help="저장해 둔 폴더를 지운다 (자동 탐색으로 돌아간다)")

    p_cs = sub.add_parser("cars",
                          help="설치 폴더의 차량 정보를 동기화하거나 보여 준다 "
                               "(면 탭 구성·상한 — 게임은 안 건드린다)")
    p_cs.add_argument("--sync", action="store_true",
                      help="설치 폴더를 다시 훑어 차량 색인을 뜬다 (`work/state/cars.json`)")
    p_cs.add_argument("--car", default=None, metavar="MAKE_Model_YY",
                      help="그 차의 면 구성만 보여 준다")

    p_ov = sub.add_parser("overlay", help="수동 따라 그리기 오버레이 실행")
    p_ov.add_argument("plan", help="도안 또는 원본 이미지(PNG/JPG) 경로 — "
                                   "이미지를 주면 원본 참조 모드로만 실행")

    p_run = sub.add_parser("run", help="도안 자동 그리기 (진행 저장·재개, STOP 파일로 중단)")
    p_run.add_argument("plan", help="도안 경로")
    p_run.add_argument("--start", type=int, default=None, help="시작 레이어 인덱스 (기본: 진행 파일에서 이어서)")
    p_run.add_argument("--limit", type=int, default=None, help="이번 실행 최대 레이어 수")

    p_sort = sub.add_parser("sortplan", help="플랜을 도형·색 그룹 연속으로 안전 정렬 (렌더 동일성 검증)")
    p_sort.add_argument("plan", help="도안 경로")
    p_sort.add_argument("-o", "--out", default=None, help="출력 경로 (기본: 같은 폴더 plan_sorted.json)")

    p_prune = sub.add_parser("pruneplan",
                             help="가시 기여 0 레이어 제거 (렌더 동일 보장 절감, 검증 포함)")
    p_prune.add_argument("plan", help="도안 경로")
    p_prune.add_argument("-o", "--out", default=None,
                         help="출력 경로 (기본: 같은 폴더 plan_pruned.json)")
    p_prune.add_argument("--min-vis", type=float, default=0.0,
                         help="가시 기여 N px 이하 레이어도 제거 (0=기여 0만·렌더 동일, "
                              ">0이면 렌더 diff 통계 출력)")

    p_kfi = sub.add_parser("kfpsimport",
                           help="KFPS JSON(편집기·게임 내보내기, 생성기 finals) "
                                "→ 도안 (overlay·run·inject 전 경로에서 "
                                "그대로 쓴다)")
    p_kfi.add_argument("kfps", help="KFPS 타입코드 JSON 또는 생성기 "
                                    "*.v2.json 경로 — 자동 판별")
    p_kfi.add_argument("-o", "--out", required=True,
                       help="출력 폴더 (<폴더이름>.plan.json·<폴더이름>.preview.png)")
    p_kfi.add_argument("--image", default=None,
                       help="원본 이미지 — 오버레이 원본 모드에 쓰인다")

    p_kfe = sub.add_parser("kfpsexport",
                           help="도안 → KFPS 타입코드 JSON (KFPS 편집기"
                                "에서 열어 고치고 kfpsimport로 되가져온다 — "
                                "도형 word·마스크까지 그대로)")
    p_kfe.add_argument("plan", help="도안(*.plan.json) 경로 (노선 무관)")
    p_kfe.add_argument("-o", "--out", default=None,
                       help="출력 경로 (기본: 같은 폴더 <폴더이름>.kfps.json)")

    p_fe = sub.add_parser("flsexport",
                          help="도안·이타샤 구성 → **게임이 읽는 파일** "
                               "(LayerGroup_*/C_group 또는 Livery_*/C_livery + "
                               "header) + FLS 편집기가 여는 .3so. 창 조작 없이 "
                               "게임에 넣는 길이다")
    p_fe.add_argument("plan", help="도안(*.plan.json, 비닐 그룹) 또는 "
                                "구성(*.itasha.json, 리버리)")
    p_fe.add_argument("-o", "--out", default=None,
                      help="컨테이너 폴더를 놓을 자리 (기본: 입력 파일 옆). "
                           "게임 저장 컨테이너 뿌리를 주면 바로 그리드에 뜬다")
    p_fe.add_argument("--name", default=None,
                      help="저장 이름 (기본: 폴더 이름)")
    p_fe.add_argument("--open", action="store_true",
                      help="쓰고 나서 FLS 편집기로 연다")

    p_fi = sub.add_parser("flsimport",
                          help="FLS·게임 파일(.3so · C_group · C_livery) → "
                               "도안. 리버리는 면마다 도안 + *.itasha.json으로 "
                               "펴서 [Itasha] 메뉴가 그대로 문다")
    p_fi.add_argument("path", help=".3so · C_group · C_livery 파일 또는 그 폴더")
    p_fi.add_argument("-o", "--out", default="out/flsimport",
                      help="출력 뿌리 (기본: out/flsimport)")

    p_fx = sub.add_parser(
        "flsedit",
        help="내장 FLS 편집기의 [Itasha] 메뉴가 부르는 엔진 — 리버리 "
    ...
    p_fx.add_argument("action",
                      ...
                      help="load-design(도안 올리기) · auto-place(자동 자리) · "
    ...
    p_fx.add_argument("--project", required=True,
                      help="편집기가 저장한 `.3so` 경로")
    p_fx.add_argument("--geometry", default=None,
                      help="그 차의 면 기하 덤프(`<미디어명>.fsgeom`) — 편집기에 "
                           "올라간 차 모델이 곧 이타샤의 차다")
    p_fx.add_argument("--slot", type=int, default=None,
                      help="지금 보고 있는 구획 번호 (0~10). 안 주면 면을 "
                           "안 가리는 명령으로 돈다")
    p_fx.add_argument("--group", nargs="*", default=None, metavar="이름",
                      help="편집기에서 **고른 그룹** 이름 (`FS:decal-1-fit` 따위) "
                           "— 안 주면 보고 있는 면의 도안이 대상이다")
    p_fx.add_argument("--design", default=None,
                      help="load-design이 올릴 도안 — *.plan.json · KFPS JSON · "
                           "`.3so`(비닐 그룹) · C_group 아무거나")
    p_fx.add_argument("--format", choices=("fls", "kfps", "plan"), default="plan",
                      help="export-group의 갈래")
    p_fx.add_argument("--color", default=None, metavar="#RRGGBB",
                      help="base-paint 색 (안 주면 도안에서 고른다)")
    p_fx.add_argument("--family", default=None,
                      help="motif 계열 — star · flower · splat · swirl · crystal "
                           "(안 주면 도안의 테마색이 고른다)")
    p_fx.add_argument("-o", "--out", default=None,
                      help="export·export-group이 쓸 자리")

    p_ed = sub.add_parser("edit",
                          help="내장 KFPS 편집기를 연다 — 도안을 브라우저 "
